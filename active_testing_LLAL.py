...
def main(args):
    ...
    model_data_type = args.model_data_type + "_LLAL"
    ...
    vit_base_path = "../Learning-Loss-for-Active-Learning/output"
    create_folder_if_not_exists(result_json_path)
    ...
    for train_step in train_steps:
        val_estimated_loss = np.array(read_json_results(f"{vit_base_path}/LLAL_{args.model_data_type}_{args.loss_range}_{data_type}_losses_{train_step}.json")['losses'])
        file_path = result_json_path + f"LLAL_all_runs_{train_step}.json"
        active_testing(file_path, true_losses, val_estimated_loss, "LLAL", sample_size_set, store_idxs=False)
