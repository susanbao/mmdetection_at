...
def take_data_for_defined_point(model_dataset, steps, image_path, result_path, level):
    ...
    base_path = f"../DE-DETRs/pro_data/{model_origin_folder}/val/"
...
if False:
    ...
    dataset_model = "DEDETR_CITY"
    step_array = np.arange(1000, 10001, 1000)
    for step in step_array:
        show_image_all_methods_mean_std(dataset_model, step)
    
    dataset_model = "DELADETR_CITY"
    step_array = np.arange(1000, 10001, 1000)
    for step in step_array:
        show_image_all_methods_mean_std(dataset_model, step)
...
# ipdb.set_trace()
if True:
    detr = take_image_data_for_defined_point("DETR_CITY", 6000)
    dfdetr = take_image_data_for_defined_point("DEDETR_CITY", 6000)
    dino = take_image_data_for_defined_point("DELADETR_CITY", 5000)
...
if True:
    ...
    plot_figure(results, "Det_CITY_Image", find_dff = True)
...
if True:
    ...
    dino['sample_size'] = detr['sample_size']
    results =  pd.concat([detr, dfdetr, dino], ignore_index=True)
    plot_figure(results, "Det_CITY_Region", find_dff = True)